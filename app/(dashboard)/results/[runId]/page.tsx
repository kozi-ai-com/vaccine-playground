  ...
  return (
    <div className='space-y-6'>
      <div className='flex flex-wrap items-start justify-between gap-4'>
        <div>
          ...
          <p className='text-sm text-muted-foreground mt-1'>
            {c.sequence_length} residues · {fmt(res.timing.total_seconds)}
          </p>
        </div>
        <div className='flex gap-2'>
          ...
          <Button onClick={() => router.push("/playground")}>
            <IconPlus className="size-5" />New Run
          </Button>
        </div>
      </div>
      ...
      <div className='grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-5'>
        {[
          { l: 'CTL Epitopes', v: c.ctl_count, s: `${c.ctl_strong} strong` },
          { l: 'HTL Epitopes', v: c.htl_count },
          { l: 'B-Cell', v: c.bcell_count },
          { l: 'Global Coverage', v: `${c.global_coverage_pct.toFixed(1)}%` },
          { l: 'African Coverage', v: `${c.african_coverage_pct.toFixed(1)}%` },
        ].map((m) => (
          <Card key={m.l}>
            <CardContent className='p-4'>
              <p className='text-xs text-muted-foreground'>{m.l}</p>
              <p className='text-2xl font-bold mt-1 tabular-nums'>{m.v}</p>
              {m.s && <p className='text-xs text-muted-foreground'>{m.s}</p>}
            </CardContent>
          </Card>
        ))}
      </div>
      ...
      <div>
        <h3 className='text-lg font-semibold mb-3'>Top Epitopes</h3>
